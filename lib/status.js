...
StatusController.prototype.api_getmoneysupply = function(req, res) {
  var self = this;
  ...
  this.node.services.bitcoind.getInfo(function(err, info) {
    ...
    self.node.services.bitcoind.coinSupply(function(err, coinsupply) {
      if (err) {
        return self.common.handleErrors(err, res);
      }
      
      // Return only the total as a number
      res.jsonp(coinsupply.total);
    });
  });
